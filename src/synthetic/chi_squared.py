...
from src.utils.synthetic_utils import chi_squared_proposal_sample as proposal_sample
from src.utils.synthetic_utils import chi_squared_IS_ratio as ratio_IS
...
if is_notebook():
    ...
    parser.add_argument(
        "--seed", "-s", default=10, type=int, help="Seed value to use.",
    )
    parser.add_argument(
        "--params_min",
        "-min",
...
MEAN_VECTOR = np.ones_like(PARAMS_VECTOR)  # Values to be estimated
