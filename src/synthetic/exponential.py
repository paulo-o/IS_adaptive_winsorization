...
from src.utils.synthetic_utils import exponential_proposal_sample as proposal_sample
from src.utils.synthetic_utils import exponential_IS_ratio as IS_ratio
...
if is_notebook():
    ...
    parser.add_argument(
        "--seed", "-s", default=1, type=int, help="Seed value to use.",
    )
    parser.add_argument(
        "--params_min",
        "-min",
...
MEAN_VECTOR = PARAMS_VECTOR  # Values to be estimated in each parameter config
...
for example_n, parameter in enumerate(PARAMS_VECTOR):
    ...
    for sim in trange(
        ...
        x = proposal_sample(N)
        y = np.asarray(IS_ratio(x, PARAMS_VECTOR[example_n]))
...
np.sqrt(N) * np.array([0.25, 0.5, 1, 1.5, 2])
